...
function LinkTreeDashboard() {
  ...
  useEffect(() => {
    if (Object.keys(data).length > 0) {
      setLinks(data.links.map((i, index) => {
        i["id"] = index;
        return i;
      }));
  ...
  return (<div>
    {successMsg == "" ? null : <div className={"success"}>{successMsg}</div>}
      ...
      <div>
        {links.map((i, index) => {
          return (<div key={index}>
            ...
            />
            <input
              onChange={(e) => handleChange(i, e.target.value, "url")}
              value={i.url}
              placeholder={"url"}
            />
          </div>);
