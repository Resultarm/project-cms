...
function Menu() {
  ...
  const submitMenu = (e) => {
    e.preventDefault();
    api.post("/create-menu", { id: id, links: menu }, {
          ...
          return (
            <div key={index}>
              <input
                ...
                placeholder={"Name"}
                value={i.name}
                onChange={(e) => change(e.target.value, "name", index)}
              />
              <input
                type={"text"}
                placeholder={"Url"}
                value={i.url}
                onChange={(e) => change(e.target.value, "url", index)}
              />
              <button onClick={(e) => deleteItem(e, index)}>Apagar</button>
            </div>
          );
        })}
