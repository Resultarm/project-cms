    ...
    <form className={"cards"} onSubmit={handleSubmit}>
      <div>
        <input
          ...
          value={cards.title}
          onChange={(e) =>
            setCards((prev) => ({ ...prev, title: e.target.value }))}
        />
      </div>
      <div>
        <input
          ...
          value={cards.content}
          onChange={(e) =>
            setCards((prev) => ({ ...prev, content: e.target.value }))}
        />
      </div>
      <div>
        <h4>Card</h4>
        {cards.cards.map((i, index) => (
          <div key={index}>
            <div style={{ display: "flex", flexDirection: "row" }}>
              <div>
                <input
                  ...
                  onChange={(e) =>
                    setCards((prev) => ({
                      ...prev,
                      cards: cards.cards.map((j, number) => {
                        if (number === index) {
              ...
              <div>
                <button onClick={() => deleteCard(index)} type={"button"}>
                  Apagar o card
                </button>
              </div>
            </div>
