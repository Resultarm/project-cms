...
function MultiSection() {
  ...
  useEffect(() => {
    api.get("/read-multi-sections").then((r) => {
      setMainSection(r.data);
    }).catch((e) => {
      if (e.response.status === 404) {
        setMainSection({ title: "", content: "" });
      }
    });
  }, []);

  useEffect(() => {
    api.get("/read-multi-sections-item").then((r) => {
      setMainSectionItem(r.data.map((i) => {
        i.image = "";
        i.created = true;
        return i;
      }));
  ...
  const deleteItem = (e, item) => {
    ...
    setMainSectionItem((prev) => (prev.filter((i) => {
      ...
      <form className={"menu-section"} onSubmit={handleMainSectionSubmit}>
        <div>
          <input
            ...
            onChange={(e) =>
              setMainSection((prev) => ({ ...prev, title: e.target.value }))}
          />
        </div>
        <div>
          <input
            ...
            onChange={(e) =>
              setMainSection((prev) => ({ ...prev, content: e.target.value }))}
          />
        </div>
        <button type={"submit"}>
          Guardar alterações
        ...
        {mainSectionItem.map((i, index) => {
          return (
            <div key={index}>
              ...
              <div>
                ...
                {i.image_path === ""
                  ? (
                    <input
                      type={"file"}
                      value={i.image === "" ? i.image : i.image.filename}
                      onChange={(e) => {
                        setMainSectionItem((prev) =>
                          prev.map((i, number) => {
                            if (index === number) {
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setMainSectionItem((prev) =>
                        prev.map((i, number) => {
                          if (index === number) {
                            i.button_link.name = e.target.value;
                          }
                          return i;
                        })
                      )}
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setMainSectionItem((prev) =>
                        prev.map((i, number) => {
                          if (index === number) {
                            i.button_link.url = e.target.value;
                          }
                          return i;
                        })
                      )}
              ...
              {!i.created
                ? <button onClick={(e) => createItem(e, i)}>Criar</button>
                : null}
              <button onClick={(e) => deleteItem(e, i)}>Apagar</button>
            </div>
          );
        })}
      </form>
